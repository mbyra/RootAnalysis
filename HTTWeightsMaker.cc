...
//  bool tauID = aTau.tauID(byMediumCombinedIsolationDeltaBetaCorr3Hits);
  ...
  bool extraRequirements =  true; //nJets30==0; // aMuon.mt()<40 &&
  ...
  //Fill empty histos and "Plus" "Minus" control histos
    if(aMuon.charge()==1 && aMuon.mt()<40 && aMuon.iso()<0.1)  {
...
//      myHistos_->fill1DHistogram("h1DMassTransDiff"+hNameSuffixN,aMuon.mt(),eventWeight);}
  ...
  if(qcdSelectionSS){
    ...
    if(aMuon.mt()<40 && aMuon.charge()==1) myHistos_->fill1DHistogram("h1DIso"+hNameSuffix+"Plus",aMuon.iso(),eventWeight);
    if(aMuon.mt()<40 && aMuon.charge()==-1) myHistos_->fill1DHistogram("h1DIso"+hNameSuffix+"Minus",aMuon.iso(),eventWeight);
    ...
    if(aMuon.mt()<40 && aMuon.iso()<0.1 && aMuon.charge()==1) fillControlHistos(eventWeight, hNameSuffixP);
    if(aMuon.mt()<40 && aMuon.iso()<0.1 && aMuon.charge()==-1) fillControlHistos(eventWeight, hNameSuffixN);
