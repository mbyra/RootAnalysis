...
HTTAnalyzer::HTTAnalyzer(const std::string & aName):Analyzer(aName){
  ...
  filePath = "AsymmWeights.root";
  asymmFile_ = new TFile(filePath.c_str());

  hMTAsymmOS = (TH1F*)asymmFile_->Get("MassTransAsymmetryOS");
  hMTAsymmSS = (TH1F*)asymmFile_->Get("MassTransAsymmetrySS");
  hMTAsymmAll = (TH1F*)asymmFile_->Get("MassTransAsymmetryAll");
...
// it has to weight all samples to calculate Delta. In plot stack use other than W+Jet sample not weighted by that.
  ...
  int iBin;
  float weightOS=0, weightSS=0, weightAll=0;

    iBin = hEtaAsymmSS->FindBin(aMuon.eta());
    weightSS = hEtaAsymmSS->GetBinContent(iBin);
    weightSS = 1/weightSS;

    iBin = hEtaAsymmOS->FindBin(aMuon.eta());
    weightOS = hEtaAsymmOS->GetBinContent(iBin);
    weightOS = 1/weightOS;
  ...
  return std::make_pair(weightSS,weightAll);
}
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
std::pair<float,float> HTTAnalyzer::getAsymmMTWeight(const EventProxyHTT & myEventProxy){
  ...
  float weightOS=0, weightSS=0, weightAll=0;
    ...
    weightSS = hMTAsymmSS->GetBinContent(iBin);

    if(weightSS!=0){
    weightSS = 1/weightSS;}

    iBin = hMTAsymmOS->FindBin(aMuon.mt());
    weightOS = hMTAsymmOS->GetBinContent(iBin);
...
//  bool tauID = 1;
  ...
  bool extraRequirements = true; //aMuon.mt()<40; // && (nJets30==0);
  ...
  bool wSelection = aMuon.mt()>70 && aMuon.iso()<0.1;
...
// Suffixes for histograms weighted by asymmetry
  std::string hNameSuffixAsymmEta = sampleName + "AsymmEta";
// .first - OS or SS samples; .second - All sample
  float asymmWeightEta = getAsymmEtaWeight(myEventProxy).first; // this weight is used only to fill some histograms
  float eventWeightAsymmEta = eventWeight*asymmWeightEta;

  std::string hNameSuffixAsymmMT = sampleName + "AsymmMT";
// .first - OS or SS samples; .second - All sample
  float asymmWeightMT = getAsymmMTWeight(myEventProxy).first; // this weight is used only to fill some histograms
  float eventWeightAsymmMT = eventWeight*asymmWeightMT;

  ///Fill variables stored in TTree
  muonPt = aMuon.pt();

  std::string hNameSuffixAsymmEtaP = hNameSuffixAsymmEta+"Plus";
  std::string hNameSuffixAsymmEtaN = hNameSuffixAsymmEta+"Minus";
  std::string hNameSuffixAsymmMTP = hNameSuffixAsymmMT+"Plus";
  std::string hNameSuffixAsymmMTN = hNameSuffixAsymmMT+"Minus";
