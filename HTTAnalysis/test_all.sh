...
run_tests() {
	...
	for (( i=1; $i <= $T_THREADS; i++ )); do
		...
		for (( j=1; $j <= $N_TESTS; j++ )); do
			...
			time -o $path/test_$j/time.txt ./test $path/htt.ini >$path/test_$j/output.txt 2>$path/test_$j/output.txt #run test with output redirected also to file in test directory
